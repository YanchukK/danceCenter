                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div class="d-flex justify-content-between flex-sm-column flex-md-row flex-wrap">
                            {{ link_to_route('news.create', 'Create news', null, ['class' => 'rounded-0 btn btn-outline-info btn-lg btn-block']) }}
                            @foreach ($newses as $model)
                                <div class="rounded-0 card mt-3" style="width: 22rem;">
                                    <img class="card-img-top"
                                         src="/storage/img/news/{{$model->news_img}}"
                                         alt="Card image cap">
                                    <div class="card-body">
...
{{--                                            {{ link_to_route('style.show', 'View', $model->id, ['class' => 'btn btn-success rounded-0 h-50 px-4']) }}--}}
                                            {{ link_to_route('news.edit', 'Update', $model->id, ['class' => 'btn btn-primary rounded-0 h-50 px-4']) }}
                                            {{Form::open(['class' => 'confirm-delete', 'route' => ['news.destroy', $model->id], 'method' => 'DELETE'])}}
